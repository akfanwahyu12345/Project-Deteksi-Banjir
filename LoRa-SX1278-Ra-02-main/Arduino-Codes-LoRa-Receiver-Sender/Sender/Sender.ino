...
// int pulseCount = 0;          // Variabel untuk menghitung pulse
...
void setup() {
  ...
  if (!LoRa.begin(LORA_FREQUENCY)) {
    Serial.println("LoRa Error");
    // loRaInitialized = false;  // Set flag to false if LoRa fails to initialize
  } else {
    Serial.println("lora ok");
    // loRaInitialized = true;  // Set flag to true if LoRa is initialized successfully
  }
...
float readUltrasonicSensor() {
  ...
  long duration = pulseIn(ECHO_PIN, HIGH);
  float distance = (duration / 2.0) * (SOUND_VELOCITY / 10000.0);
  return distance;  // Kembalikan jarak tanpa offset tambahan
}
